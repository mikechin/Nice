...
- **Depth biases the *draw* and tilts the *loot roll* — never base power or grade odds.** Deeper rooms pull **lower-stability, less-mastered characters** as prompts (harder reviews) and **tilt the rarity roll upward** on the loot they drop. Both are biases on RNG, not power handouts.
- **Rarity = a roll at drop time, NOT an HSK tier (revised 2026-06-03; supersedes "rarity = HSK tier, fixed per character").** Every loot drop *rolls* its rarity (common→epic) with a **nonzero floor for every tier from drop #1**, weighted up by **clutch recall** (nailing an ABOUT_TO_FORGET / low-retrievability card), **depth**, and **streak/crit**. Rarity is a property of the *instance you pulled*, not of the character — the same 你 can drop common today and rare tomorrow. It sets **shard/craft value** and **the grade-band ceiling** (see Grading); it does **not** scale base power and it does **not** pick the ability family (the *crafting connection* does that — see Crafting). Card identity stays **rarity × grade** (TCG-style print-rarity × condition: a pristine PSA-10 common and a beat-up rare both exist). **Why the change:** HSK→rarity gave beginners no variable-ratio dopamine (months of nothing but commons), inverted the populations (HSK 4 has *more* words than HSK 1, so "epic" would be the most numerous tier), and rested on the false premise that higher-HSK words are more powerful — the low-HSK workhorses (是, 的, 你) are the highest-utility. **HSK is now the curriculum/pacing axis only** (what's *due*) and at most a soft odds weight, never a hard rarity gate.
- **Grade = the fast/fun variance layer, and the *loud* dopamine.** Base is flat and honest; the **PSA roll** (loaded by mastery, capped by the rolled rarity) is the concentrated reward beat. Base = floor, grade roll = excitement and ceiling. (Drops stay deliberately *quiet* so grading carries the jackpot — see Dopamine Pacing under Grading.)
...
- **Two caps on the band: rarity sets the *ceiling*, mastery sets the *reach* within it. LOCKED.** **The instance's rolled rarity caps how high it can ever grade** — a common-rolled instance tops out low, an epic-rolled one can reach PSA 10. **Mastery (FSRS stability) sets where in that range you actually land** — low stability can't approach the ceiling even on a rare instance; the craft's RNG rolls within. Effective top = `min(rarity-cap, mastery-cap)`, so **PSA 10 needs both a rare-rolled instance *and* real mastery** — that's the min/max chase (luck on the drop × skill in the learning). Preserves "easy/new cards stay weak even when graded," now from two directions. (Band-over-soft-odds + the two-cap structure confirmed 2026-06-02; rarity redefined from HSK-tier to drop-roll 2026-06-03.)
- **Teaching hook — show the odds, and foreshadow at the drop.** The grading screen previews your chances (*"your mastery of 水 → 60% PSA 8+"*), making the learn→reward link visible and nudging more reps before grading. Because mastery is known the moment a card drops, the band ceiling is **foreshadowable in the dungeon** too — a quiet "this one feels special" hint when a high grade is within reach. The honest backend becomes a *visible* motivator.
- **High PSA is earned, not rolled (the early-dopamine answer).** Because the band is mastery-capped, a beginner literally can't pull a gem-mint 10 yet — correct, and on-invariant. Their grading carrot is the **ceiling rising as they master**: grade 我 today → PSA 5 (mastery-capped); master it, pull a future 我 instance, grade it → PSA 9; the binder's *best-PSA* climbs. No re-grade mechanic needed — it falls out of the drop economy. The drop-time hint doubles as a study nudge ("mastery-limited — review this to reach gem-mint").
- **Dopamine pacing — drop quiet, grade loud, answer often.** Drops are frequent in an extraction loop, so each one is *low-key* (you carried out a raw card); the **grade** is the concentrated slot-machine pull (deliberate, town-only). Early-game dopamine rides on **answering**: NEW reveals are rationed (~3/run via `MAX_NEW_CARDS_PER_SESSION`), LEARNING dominates a beginner's deck, ABOUT_TO_FORGET is the clutch-rescue hit. Caveat: a caught-up player rarely sees ABOUT_TO_FORGET (cards come due at R≈0.9, above the 0.7 trigger), so their runs lean on grading + combat juice + depth-drawn low-stability rescues for the late-game beats. The comfortable-review state (`LootRarity.COMMON`) is the **silent default** — no special treatment; reserve juice for NEW / LEARNING / ABOUT_TO_FORGET.
...
- Grading/power: **base = flat per effect** (not mastery/depth/rarity-derived); **grade sets effect magnitude** (e.g. +10 → +20 max HP) via the PSA roll. **The grade band has two caps: rarity (rolled per drop) sets the ceiling, mastery (FSRS stability) sets the reach within it** — top = `min(rarity-cap, mastery-cap)`, so PSA 10 needs a rare-rolled instance *and* mastery. High PSA is **earned** (mastery-capped), so a beginner's carrot is the ceiling climbing as they learn. **Dopamine pacing: drop quiet, grade loud** — answering carries early-game dopamine (NEW rationed, LEARNING dominant for beginners, ABOUT_TO_FORGET = clutch-rescue but sparse once caught up).
- Rarity: **= a roll at drop time (revised 2026-06-03; was "HSK tier, intrinsic & fixed").** Every drop rolls common→epic with a nonzero floor for all tiers from drop #1, weighted up by clutch recall (ABOUT_TO_FORGET correct) / depth / streak. Sets craft value + the grade-band ceiling — **not** base power, and **not** the ability family (the crafting connection authors that). Card identity = **rarity × grade** (TCG print-rarity × condition). HSK = curriculum/pacing only, at most a soft odds weight. The **min/max meta** = chase rare-rolled instances (luck) × grade them high via mastery (skill) × build synergy. **Why the overturn:** HSK→rarity starved beginners of dopamine, inverted tier populations (HSK 4 > HSK 1 in word count), and falsely equated high HSK with power.
