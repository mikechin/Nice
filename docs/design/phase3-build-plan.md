...
| **Collection / binder** | `src/progression/collection_grid.gd`, `player_profile.gd`, `collection_enums.gd` | `CollectionGrid` = the binder. **Retermed (D9):** per-char record = **seen / dropped / best-PSA + an unnamed mastery bar** — drop the `CardTier{COMMON…LEGENDARY}` gem names (they collide with instance rarity + PSA grades). `card_tier_calculator.gd`'s stability bands survive only as the mastery-bar fill thresholds. `PlayerProfile` tracks mastery %, runs, accuracy. **No card-instance or PSA-grade layer exists yet** — see D3. |
...
- **D2 — `PowerEnums.BASE_POWER` is inverted and must be reworked.** Today weaker cards = more board power (Triple-Triad logic). Phase 3 locks **flat base per effect; rarity does not scale base; grade (the PSA grade — see D8) sets magnitude**. Rework `power_enums.gd`/`power_calculator.gd` to the new model when M5 lands; until then combat can use a flat placeholder.
- **D3 — Three orthogonal axes — rarity (rolled), grade/PSA (earned), binder (factual record). Don't conflate them; earlier drafts welded all three to HSK/stability.**
  - **Rarity** — scarcity, the gacha axis. A property *rolled at drop time* on each `CardInstance`, NOT a fact about the character and NOT an HSK map (see D7). Caps the achievable PSA band.
  - **Grade / PSA (1–10)** — the *earned* strength of a graded instance. Set by crafting; band = `min(rarity-cap, mastery-cap)`, mastery loads where you land. The *loud* dopamine (see D8).
  - **Binder** — a permanent, factual *record* per character: seen / dropped / best-PSA-ever + an unnamed mastery bar (NO gem tiers, see D9). Knowledge, never lost.
  - `CardInstance` (character + rolled rarity + PSA grade + authored ability + provenance) is the disposable/stakeable object, stored in inventory + saved. The binder is recorded/computed, never staked.
...
- **D7 — Instance rarity is a *drop roll*, never an HSK→rarity map (supersedes M3's `RarityMap`). RESOLVED 2026-06-03.** The original HSK1/2/3/4 → common/uncommon/rare/epic map fails three ways: (1) a beginner grinding HSK1 pulls only commons for months — no variable-ratio dopamine; (2) the populations invert — HSK 1 ≈ 150 words, HSK 4 ≈ 600, so "epic" (HSK 4) would be the *most numerous* tier, backwards from every collectible; (3) "higher HSK = more powerful" is false — the low-HSK workhorses (是, 的, 你) are the highest-frequency/highest-utility words. So rarity becomes a **roll with a nonzero floor for every tier from drop #1**, weighted up by clutch recall (answering an ABOUT_TO_FORGET / low-retrievability card correctly — `get_loot_rarity` already computes the state), dungeon depth (`DepthDraw`), and streak/crit. HSK stays the *curriculum/pacing* axis and at most a soft odds weight — never a hard gate. **M3 builds `RarityRoll` (the weighted roll), not `RarityMap`.**
- **D8 — Dopamine pacing: drop quiet, grade loud; the PSA band is foreshadowable. RESOLVED 2026-06-03.** A slot-pull per drop causes reward fatigue, so spread the dopamine across the loop:
  - **Answering** carries the early game. NEW_CARD reveals are rationed (`MAX_NEW_CARDS_PER_SESSION=3`, `…PER_DAY=5`) → ~3 reliable novelty hits/run; LEARNING dominates a beginner's deck (eventful); ABOUT_TO_FORGET is the clutch-rescue hit. **Caveat:** ABOUT_TO_FORGET fires at retrievability `<0.7`, but cards are scheduled to come due at R ≈ `DEFAULT_DESIRED_RETENTION=0.9` — so it's intrinsically a "behind/returning" signal, *sparse* for a caught-up player. `DepthDraw` can deliberately pull low-R cards at depth to manufacture honest rescue moments.
  - **The comfortable-review state (`LootRarity.COMMON`) is the silent default** — no special juice; reserve treatment for NEW / LEARNING / ABOUT_TO_FORGET. (Rename → `KNOWN`/`REVIEW`; it's "you've got this," not a rarity.)
  - **The drop is quiet** ("you carried out a raw card"); **grading is the concentrated slot-machine pull** (deliberate, town-only).
  - **PSA band is predictable at drop time.** Band = `min(rarity-cap, mastery-cap)`; mastery (FSRS stability) is known when the card drops, so the ceiling can be foreshadowed ("this one feels special — could grade high"). The roll *within* the band is the suspense; the ceiling is the earned part.
  - **High PSA is earned, not rolled.** Mastery-capped band ⇒ a beginner can't pull a gem-mint 10 yet (correct, on-invariant). Their carrot is the **ceiling rising as they master**: grade 我 today → PSA 5; master it, grade a future 我 instance → PSA 9; the binder's best-PSA climbs. No re-grade mechanic — it falls out of the drop economy. The foreshadow hint doubles as a **teaching hook** ("mastery-limited — review more to reach gem-mint").
  - **Veteran-COMMON risk:** a caught-up veteran's run is mostly the quiet default state, so answering-dopamine thins — grading + combat juice + `DepthDraw` rescues must carry the late game.
- **D9 — Binder drops the COMMON→LEGENDARY tier names. RESOLVED 2026-06-03.** They collide with instance rarity (common/…/epic) and PSA grades — three gem-scales for three different things. The binder becomes a factual per-char record: **seen** (encountered), **dropped** (ever owned an instance), **best PSA** (peak grade ever — a permanent trophy that survives losing the instance), and **mastery** as an unnamed bar/%. Retires the `CardTier{COMMON…LEGENDARY}` *naming* for the binder; `card_tier_calculator.gd`'s stability bands survive only as the (unnamed) mastery-bar fill thresholds. Completion goal becomes "best-PSA every char," which is purely mastery-honest.
...
src/
  ...
  cards/       CardInstance, Inventory (the bench), GradeBand, RarityRoll (weighted drop roll, not HSK — D7)
...
- **New:** `CardInstance`, `Inventory` (bench), `RarityRoll` (weighted drop roll — clutch-recall/depth/streak, NOT HSK→rarity; see D7), `GradeBand`, `Currency` (shards); `SaveManager` schema bump (instances + shards + unlocks). **Also:** reterm the binder to seen/dropped/best-PSA + mastery bar (D9); make `LootRarity.COMMON` the silent default (D8).
- **Reuse:** binder (`CollectionGrid`) record reused (retermed, not rebuilt); raw instance = `CharacterData` + rolled rarity + ungraded.
- **Proves:** the three-axis model (D3/D7) and the loss floor = instances, never binder.
...
- **New:** `town.tscn` + `home.tscn`/`crafter.tscn`/`shop.tscn`; `Loadout` (5 slots, 2 passive/3 active, POS split from `CharacterData` part-of-speech), `Stake`; `CraftSystem` (pick target + 3 connected instances + shard fee → PSA roll where **mastery loads the band** = `min(rarity-cap, mastery-cap)`, foreshadowable at drop time per D8, **connection authors the ability**), `ConnectionSet`, `Shop`.
